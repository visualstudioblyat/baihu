...
/// Skills live in `~/.baihu/workspace/skills/<name>/SKILL.md`
    ...
    if !readme.exists() {
        std::fs::write(
            ...
            "# Baihu Skills\n\n\
             ...
             ```bash\n\
             baihu skills install <github-url>\n\
             baihu skills list\n\
             ```\n",
        )?;
    }
    ...
    match command {
        super::SkillCommands::List => {
            ...
            if skills.is_empty() {
                ...
                println!();
                println!("  Create one: mkdir -p ~/.baihu/workspace/skills/my-skill");
                println!(
                    "              echo '# My Skill' > ~/.baihu/workspace/skills/my-skill/SKILL.md"
                );
                println!();
                println!("  Or install: baihu skills install <github-url>");
            } else {
        ...
        super::SkillCommands::Install { source } => {
            ...
            if source.starts_with("http") || source.contains("github.com") {
                ...
                if output.status.success() {
                    ...
                    println!("  Restart `baihu channel start` to activate.");
                } else {
...
"#,
    ...
    #[test]
    fn skills_dir_path() {
        let base = std::path::Path::new("/home/user/.baihu");
        let dir = skills_dir(base);
        assert_eq!(dir, PathBuf::from("/home/user/.baihu/skills"));
    }
