...
///
/// Follows the workspace framework structure:
...
pub fn build_system_prompt(
    ...
    if prompt.is_empty() {
        "You are Baihu, a fast and efficient AI assistant built in Rust. Be helpful, concise, and direct.".to_string()
    } else {
        prompt
    }
}

fn inject_identity(prompt: &mut String, workspace_dir: &std::path::Path) {
...
pub fn handle_command(command: super::ChannelCommands, config: &Config) -> Result<()> {
    match command {
        ...
        super::ChannelCommands::List => {
            ...
            println!("\nTo start channels: baihu channel start");
            println!("To check health:    baihu channel doctor");
            println!("To configure:      baihu onboard");
            Ok(())
        }
        super::ChannelCommands::Add {
            ...
            anyhow::bail!(
                "Channel type '{channel_type}' — use `baihu onboard` to configure channels"
            );
        }
        super::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.baihu/config.toml directly");
        }
    }
}
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No real-time channels configured. Run `baihu onboard` first.");
        return Ok(());
    }

    println!("🩺 Baihu Channel Doctor");
    println!();
    ...
    if config.channels_config.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `baihu gateway` then GET /health");
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No channels configured. Run `baihu onboard` to set up channels.");
        return Ok(());
    }

    println!("🦀 Baihu Channel Server");
    println!("  🤖 Model:    {model}");
    println!(
        "  🧠 Memory:   {} (auto-save: {})",
...
mod tests {
    ...
    fn prompt_injects_workspace_files() {
        ...
        assert!(prompt.contains("Name: Baihu"), "missing IDENTITY content");
