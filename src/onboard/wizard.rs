...
pub fn run_wizard() -> Result<Config> {
    ...
    println!(
        "  {}",
        style("Welcome to Baihu — the fastest, smallest AI assistant.")
            .white()
            .bold()
    );
    println!(
    ...
    print_step(3, 8, "Channels (How You Talk to Baihu)");
    ...
    if has_channels && config.api_key.is_some() {
        ...
        if launch {
            ...
            std::env::set_var("BAIHU_AUTOSTART_CHANNELS", "1");
        }
    }
...
pub fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "Channels (How You Talk to Baihu)");
...
/// Use `baihu onboard` or `baihu onboard --api-key sk-... --provider openrouter --memory sqlite`.
/// Use `baihu onboard --interactive` for the full wizard.
...
pub fn run_quick_setup(
    ...
    let workspace_dir = baihu_dir.join("workspace");
    ...
    if api_key.is_none() {
        ...
        println!("    2. Or edit:           ~/.baihu/config.toml");
        println!("    3. Chat:              baihu agent -m \"Hello!\"");
        println!("    4. Gateway:           baihu gateway");
    } else {
        println!("    1. Chat:     baihu agent -m \"Hello!\"");
        println!("    2. Gateway:  baihu gateway");
        println!("    3. Status:   baihu status");
    }
    println!();
...
fn setup_workspace() -> Result<(PathBuf, PathBuf)> {
    let home = directories::UserDirs::new()
        .map(|u| u.home_dir().to_path_buf())
        .context("Could not find home directory")?;
    let default_dir = home.join(".baihu");
...
fn setup_provider() -> Result<(String, String, String)> {
    ...
    if providers.is_empty() {
        ...
        print_bullet("Baihu works with ANY API that speaks the OpenAI chat completions format.");
...
fn setup_tool_mode() -> Result<(ComposioConfig, SecretsConfig)> {
    print_bullet("Choose how Baihu connects to external apps.");
    ...
    let composio_config = if choice == 1 {
        ...
        print_bullet("Baihu uses Composio as a tool — your core agent stays local.");
        println!();
    ...
    // ── Encrypted secrets ──
    println!();
    print_bullet("Baihu can encrypt API keys stored in config.toml.");
...
fn setup_memory() -> Result<MemoryConfig> {
    print_bullet("Choose how Baihu stores and searches memories.");
    print_bullet("You can always change this later in config.toml.");
    println!();
...
fn setup_channels() -> Result<ChannelsConfig> {
    print_bullet("Channels let you talk to Baihu from anywhere.");
        ...
        match choice {
            0 => {
                ...
                println!(
                    ...
                    style("— talk to Baihu from Telegram").dim()
                );
            ...
            1 => {
                ...
                println!(
                    ...
                    style("— talk to Baihu from Discord").dim()
                );
            ...
            2 => {
                ...
                println!(
                    ...
                    style("— talk to Baihu from Slack").dim()
                );
            ...
            3 => {
                ...
                print_bullet("Baihu reads your iMessage database and replies via AppleScript.");
                print_bullet(
            ...
            5 => {
                ...
                let verify_token: String = Input::new()
                    ...
                    .default("baihu-whatsapp-verify".into())
                    .interact_text()?;
...
fn print_summary(config: &Config) {
    ...
    println!(
        "  {}  {}",
        style("⚡").cyan(),
        style("Baihu is ready!").white().bold()
    );
    println!(
        "  {}",
        style("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━").cyan()
    ...
    if has_channels {
        ...
        println!("       {}", style("baihu channel start").yellow());
        println!();
        step += 1;
    }
    ...
    println!(
        "       {}",
        style("baihu agent -m \"Hello, Baihu!\"").yellow()
    );
    println!();
    step += 1;
    ...
    println!("       {}", style("baihu agent").yellow());
    println!();
    step += 1;
    ...
    println!("       {}", style("baihu status").yellow());
...
mod tests {
    ...
    fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = fs::read_to_string(tmp.path().join("IDENTITY.md")).unwrap();
        assert!(
            identity.contains("**Name:** Baihu"),
            "should default agent name to Baihu"
        );
    ...
    fn scaffold_full_personalization() {
        ...
        let identity = fs::read_to_string(tmp.path().join("IDENTITY.md")).unwrap();
        assert!(identity.contains("**Name:** Baihu"));

        let soul = fs::read_to_string(tmp.path().join("SOUL.md")).unwrap();
        assert!(soul.contains("You are **Baihu**"));
        ...
        let agents = fs::read_to_string(tmp.path().join("AGENTS.md")).unwrap();
        assert!(agents.contains("Baihu Personal Assistant"));

        let bootstrap = fs::read_to_string(tmp.path().join("BOOTSTRAP.md")).unwrap();
        assert!(bootstrap.contains("**Argenis**"));
        assert!(bootstrap.contains("US/Eastern"));
        assert!(bootstrap.contains("Introduce yourself as Baihu"));

        let heartbeat = fs::read_to_string(tmp.path().join("HEARTBEAT.md")).unwrap();
        assert!(heartbeat.contains("Baihu"));
    }
