...
impl OpenAiCompatibleProvider {
    pub fn new(name: &str, base_url: &str, api_key: Option<&str>, auth_style: AuthStyle) -> Self {
        Self {
            ...
            client: super::http_client::build_ssrf_safe_client(),
        }
    }
}
...
impl Provider for OpenAiCompatibleProvider {
    async fn chat_with_system(
        ...
        let api_key = self.api_key.as_ref().ok_or_else(|| {
            anyhow::anyhow!(
                "{} API key not set. Run `baihu onboard` or set the appropriate env var.",
                self.name
            )
        })?;
        ...
        // SSRF: block requests to private/internal networks
        if let Err(reason) = super::http_client::validate_url_not_private(&url) {
            anyhow::bail!("{} SSRF blocked: {reason}", self.name);
        }
    ...
    fn request_serializes_correctly() {
        let req = ChatRequest {
            ...
            messages: vec![
                Message {
                    role: "system".to_string(),
                    content: "You are Baihu".to_string(),
                },
                Message {
                    role: "user".to_string(),
