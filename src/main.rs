...
/// `Baihu` - No overhead. No compromise. 100% Rust.
#[derive(Parser, Debug)]
#[command(name = "baihu")]
#[command(author = "visualstudioblyat")]
...
enum MigrateCommands {
    /// Import memory from a legacy workspace
    Openclaw {
        /// Path to source workspace (defaults to ~/.openclaw/workspace)
        #[arg(long)]
...
async fn main() -> Result<()> {
    ...
    } = &cli.command
    {
        ...
        if std::env::var("BAIHU_AUTOSTART_CHANNELS").as_deref() == Ok("1") {
    ...
    match cli.command {
        ...
        Commands::Gateway { port, host } => {
            if port == 0 {
                info!("🚀 Starting Baihu Gateway on {host} (random port)");
            } else {
                info!("🚀 Starting Baihu Gateway on {host}:{port}");
            }
        ...
        Commands::Daemon { port, host } => {
            if port == 0 {
                info!("🧠 Starting Baihu Daemon on {host} (random port)");
            } else {
                info!("🧠 Starting Baihu Daemon on {host}:{port}");
            }
            daemon::run(config, host, port).await
        }

        Commands::Status => {
            println!("🦀 Baihu Status");
            println!();
