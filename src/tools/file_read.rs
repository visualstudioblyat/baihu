...
mod tests {
    ...
    async fn file_read_existing_file() {
        let dir = std::env::temp_dir().join("baihu_test_file_read");
    ...
    async fn file_read_nonexistent_file() {
        let dir = std::env::temp_dir().join("baihu_test_file_read_missing");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_blocks_path_traversal() {
        let dir = std::env::temp_dir().join("baihu_test_file_read_traversal");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_empty_file() {
        let dir = std::env::temp_dir().join("baihu_test_file_read_empty");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_nested_path() {
        let dir = std::env::temp_dir().join("baihu_test_file_read_nested");
        let _ = tokio::fs::remove_dir_all(&dir).await;
    ...
    async fn file_read_blocks_symlink_escape() {
        ...
        let root = std::env::temp_dir().join("baihu_test_file_read_symlink_escape");
